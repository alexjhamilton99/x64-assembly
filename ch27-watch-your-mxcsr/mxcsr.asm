...
section .data
    ...
    f_div       db      "%.1f divide by %.1f is %.16f, in hex: ",0
...
main:
    ...
    ; division with precision error - default mxcsr
    mov     rdi, fmt1
    mov     rsi, ten
    mov     rdx, three
    mov     ecx, [default_mxcsr]
    ...
    ; division with precision error - round up
    mov     rdi, fmt4
    mov     rsi, ten
    mov     rdx, three
    mov     ecx, [round_up]
    ...
    ; division with precision error - round down
    mov     rdi, fmt5
    mov     rsi, ten
    mov     rdx, three
    mov     ecx, [round_down]
    ...
    ; division with precision error - truncate
    mov     rdi, fmt6
    mov     rsi, ten
    mov     rdx, three
    mov     ecx, [truncate]
...
apply_mxcsr:
    ...
    movsd       xmm2, [rsi]             ; copy double-precision float into xmm2
    divsd       xmm2, [rdx]
    stmxcsr     [mxcsr_after]           ; save mxcsr to memory
    movsd       [xmm], xmm2             ; for print_xmm's use
    mov         rdi, f_div
    movsd       xmm0, [rsi]
    movsd       xmm1, [rdx]
    call        printf
