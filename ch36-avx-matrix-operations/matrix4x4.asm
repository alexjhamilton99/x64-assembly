...
section .data
	...
	fmt10	db	10,"The matrix is singular!",10,10,0
...
section .bss
	...
	.mxcsr		resd	1	; checks for zero division

section .text
	push	rbp
	mov	rbp, rsp

	push		rsi		; save inverse matrix's address
	vzeroall			; clear all ymm registers

	; compute intermediate matrices
	; compute intermediate matrix2
	; rdi contains original matrix's address
	mov	rsi, rdi
	mov	rdx, .matrix2
	push	rdi
	call	multi4x4
	...
	; compute trace 2
	push	rdi			; save original matrix's address
	mov	rdi, .matrix2
	mov	rsi, [.size]
	call	vtrace
	...
	divsd		xmm13, [.two]			; divide by p2 in xmm13
	; copmute coefficient p3
	...
	vfmadd231sd	xmm15, xmm13, xmm9		; xmm15 + p2 * s1; xmm15 = xmm15 + xmm13 * xmm9
	...
	pop		rdi				; restore original matrix's address
	...
	; no 0 division
	pop		rsi			; recall inverse matrix's address
	...
	.loop2:
		...
		vmulpd		ymm0, ymm0, ymm2		; multiply the row by -1/p4
...
section .text
	...
	.loop:
		...
		shl	r15, 3			; multipl by 8
		sub	rsp, r15		; subtract 0 or 8
		call	printf
...
multi4x4:
	...
	.loop:
		...
		add		rax, 8			; one element has 64 bits
		add		rsi, 32			; every row has 256 bits

		loop		.loop

	; move result to memory, row per row
	vmovapd		[rdx], ymm12
